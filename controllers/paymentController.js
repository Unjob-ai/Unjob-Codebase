...
const createPaymentOrder = catchAsync(async (req, res, next) => {
  ...
  if (
    ...
    return next(new AppError("Valid payment type is required", 400));
  }
  ...
  switch (type) {
    ...
    case "gig_escrow":
      if (!gigId) {
        return next(new AppError("Gig ID is required for gig payments", 400));
      }
      ...
      if (gig.company._id.toString() !== req.user._id.toString()) {
        return next(
          new AppError("Not authorized to make payment for this gig", 403)
        );
      }
      break;
    ...
    case "milestone_payment":
      if (!projectId) {
        return next(
          new AppError("Project ID is required for milestone payments", 400)
        );
      }
      ...
      if (project.company._id.toString() !== req.user._id.toString()) {
        return next(
          new AppError("Not authorized to make payment for this project", 403)
        );
      }
      break;
...
const verifyPayment = catchAsync(async (req, res, next) => {
  ...
  if (
    ...
    return next(
      new AppError("All payment verification fields are required", 400)
    );
  }
...
const requestRefund = catchAsync(async (req, res, next) => {
  ...
  if (payment.payer.toString() !== req.user._id.toString()) {
    return next(new AppError("Only the payer can request a refund", 403));
  }
  ...
  if (refundAmount > payment.amount) {
    return next(
      new AppError("Refund amount cannot exceed payment amount", 400)
    );
  }
...
const updatePaymentStatus = catchAsync(async (req, res, next) => {
  if (req.user.role !== "admin") {
    return next(new AppError("Only admins can update payment status", 403));
  }
