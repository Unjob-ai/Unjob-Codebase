...
const createGig = catchAsync(async (req, res, next) => {
  if (req.user.role !== "hiring") {
    return next(new AppError("Only hiring companies can create gigs", 403));
  }
...
const applyToGig = catchAsync(async (req, res, next) => {
  if (req.user.role !== "freelancer") {
    return next(new AppError("Only freelancers can apply to gigs", 403));
  }
  ...
  if (!gig || gig.status !== "active" || !gig.isActive) {
    return next(new AppError("Gig not found or not available", 404));
  }
  ...
  if (gig.applications.length >= gig.maxApplications) {
    return next(
      new AppError("This gig is no longer accepting applications", 400)
    );
  }
...
const getGigApplications = catchAsync(async (req, res, next) => {
  ...
  if (gig.company.toString() !== req.user._id.toString()) {
    return next(new AppError("Not authorized to view applications", 403));
  }
...
const updateGig = catchAsync(async (req, res, next) => {
  ...
  // Check if user owns the gig
  if (gig.company.toString() !== req.user._id.toString()) {
    return next(new AppError("Not authorized to update this gig", 403));
  }

  // Prevent updates if gig has accepted applications
  if (gig.selectedFreelancer) {
    return next(
      new AppError("Cannot update gig with accepted applications", 400)
    );
  }
...
const deleteGig = catchAsync(async (req, res, next) => {
  ...
  // Check if user owns the gig
  if (gig.company.toString() !== req.user._id.toString()) {
    return next(new AppError("Not authorized to delete this gig", 403));
  }

  // Prevent deletion if gig has accepted applications
  if (gig.selectedFreelancer) {
    return next(
      new AppError("Cannot delete gig with accepted applications", 400)
    );
  }
...
// @access  Private
const searchGigs = catchAsync(async (req, res, next) => {
  const {
    q,
    category,
    location,
...
// @access  Private (Hiring companies only)
const getCompanyGigs = catchAsync(async (req, res, next) => {
  if (req.user.role !== "hiring") {
    return next(
      new AppError("Only hiring companies can access this endpoint", 403)
    );
  }
...
const getMyApplications = catchAsync(async (req, res, next) => {
  if (req.user.role !== "freelancer") {
    return next(new AppError("Only freelancers can access this endpoint", 403));
  }
...
// @access  Private
const getGigCategories = catchAsync(async (req, res, next) => {
...
// @access  Private
const getGigStats = catchAsync(async (req, res, next) => {
...
// @access  Private
const getFeaturedGigs = catchAsync(async (req, res, next) => {
