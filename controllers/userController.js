...
const completeProfile = catchAsync(async (req, res, next) => {
  ...
  if (user.role === "freelancer") {
    ...
    if (!profileData.skills || profileData.skills.length < 3) {
      return next(new AppError("At least 3 skills are required", 400));
    }
    ...
    if (
      ...
      return next(new AppError("Contact person name is required", 400));
    }

    if (!profileData.businessEmail || !profileData.businessEmail.trim()) {
      return next(new AppError("Business email is required", 400));
    }
  }
